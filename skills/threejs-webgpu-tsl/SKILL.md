...
3. Build the smallest renderer initialization and backend/fallback test.
...
- In current Three.js releases, distinguish WebGPURenderer's built-in WebGL 2 backend fallback from a separately maintained WebGLRenderer implementation.
- Keep a separate WebGLRenderer path only when required materials, effects, browser targets, or behavior cannot use the renderer's backend fallback.
