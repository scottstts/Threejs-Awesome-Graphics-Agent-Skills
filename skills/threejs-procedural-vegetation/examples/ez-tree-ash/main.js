...
import { compileAshTree } from "./tree-system.js";
...
renderer.setClearColor(0x94b9f8, 1);
...
const [
  ...
  textureLoader.loadAsync("./assets/bark-color.jpg"),
  textureLoader.loadAsync("./assets/bark-normal.jpg"),
  textureLoader.loadAsync("./assets/bark-roughness.jpg"),
  textureLoader.loadAsync("./assets/ash.png"),
  textureLoader.loadAsync("./assets/ground-grass.jpg"),
  textureLoader.loadAsync("./assets/ground-dirt.jpg"),
  textureLoader.loadAsync("./assets/ground-normal.jpg"),
]);
...
const gltf = await gltfLoader.loadAsync("./assets/grass.glb");
...
function attachMeadowWind(material, instanced, amplitude) {
  ...
  material.customProgramCacheKey = () =>
    `ez-tree-meadow-wind-${instanced ? "instanced" : "mesh"}-${amplitude}`;
}
...
for (const asset of [
  "./assets/flower-white.glb",
  "./assets/flower-blue.glb",
  "./assets/flower-yellow.glb",
]) {
...
exampleRuntime.onStateChange((state) => {
  runtimeState = state;
  leafOriginPoints.visible = state.debugMode === "leaf-origins";
  const final = state.debugMode === "final" || state.debugMode === "leaf-origins";

  if (state.debugMode === "hierarchy") {
    setTreeMaterials(
      foregroundTree,
      hierarchyBranchMaterial,
      hierarchyLeafMaterial,
    );
  } else if (state.debugMode === "continuations") {
    setTreeMaterials(
      foregroundTree,
  ...
  } else if (state.debugMode === "normals") {
    setTreeMaterials(
      foregroundTree,
  ...
  if (state.debugMode !== "continuations") {
...
function resize() {
  ...
  if (
    size.x !== Math.round(width * dpr) ||
    size.y !== Math.round(height * dpr)
  ) {
...
renderer.setAnimationLoop(() => {
  ...
  elapsed += exampleRuntime.frameDelta(rawDelta);
  ...
  if (metricElapsed >= 1) {
    ...
    exampleRuntime.reportMetrics({
      fps: Math.round(frames / metricElapsed),
      branches: branchJobs,
